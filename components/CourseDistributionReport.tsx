...
const EXPORT_COLUMNS = [
    ...
    { key: 'totalStudents', label: 'STUDENT' },
    { key: 'totalVacancy', label: 'VACANCY' },
];
...
export const CourseDistributionReport: React.FC<CourseDistributionReportProps> = ({ data, programData, showExportPanel = false, setShowExportPanel }) => {
    ...
    const programMap = useMemo(() => {
        ...
        programData.forEach(p => {
            if (p.PID) {
                map.set(normalize(p.PID), { 
                    faculty: p['Faculty Short Name'] || 'Other',
                    shortName: p['Program Short Name'] || p.PID
                });
            }
        });
        return map;
    ...
    const { facultyGroups, allPrograms } = useMemo(() => {
        ...
        const programs: { name: string, count: number, faculty: string }[] = [];
        const programCounts = new Map<string, { name: string, faculty: string, count: number }>();

        data.forEach(item => {
            const normalizePid = String(item.pid).replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
            const pInfo = programMap.get(normalizePid);
            const faculty = pInfo?.faculty || 'Other';
            const progName = pInfo?.shortName || item.program;
            const uniqueKey = progName;

            if (!programCounts.has(uniqueKey)) {
                programCounts.set(uniqueKey, { name: progName, faculty, count: 0 });
            }
            programCounts.get(uniqueKey)!.count += 1; 
        });

        programCounts.forEach(val => {
            if (!groups[val.faculty]) groups[val.faculty] = [];
            groups[val.faculty].push({ program: val.name, count: val.count });
            programs.push(val);
        });

        Object.keys(groups).forEach(fac => {
    ...
    const sortedFaculties = useMemo(() => 
        ['FBE', 'FE', 'FHLS', 'FHSS', 'FSIT'].filter(f => facultyGroups[f])
    , [facultyGroups]);

    useEffect(() => {
        if (sortedFaculties.length > 0 && !activeFaculty) {
            setActiveFaculty(sortedFaculties[0]);
        }
    ...
    const handleDownload = () => {
        ...
        (window as any).XLSX.utils.book_append_sheet(workbook, worksheet, "Course Summary");
    ...
    const renderFacultyCard = (fac: string, mobile: boolean = false) => {
        ...
        return (
            <div className={`bg-white rounded border shadow-sm flex flex-col overflow-hidden ${mobile ? 'w-full h-auto mb-4' : 'min-w-[300px] md:min-w-0 snap-center'}`}>
                {!mobile && (
                    <div className="py-2 md:py-1.5 px-3 md:px-2 text-center border-b font-bold text-sm md:text-xs text-gray-700 bg-gray-50">
                        {fac}
                    </div>
                )}
                ...
                <div 
                    ...
                    style={{ height: mobile ? 'auto' : `${contentHeight}px`, maxHeight: mobile ? '400px' : 'none' }}
                >
                    {programs.map(p => (
                ...
                {chartData.map((p, idx) => {
                    ...
                    return (
                        <div key={idx} className="flex-1 flex flex-col items-center justify-end group relative h-full hover:bg-gray-50/50 rounded-lg transition-colors min-w-[20px]">
                            ...
                            <div 
                                className="absolute left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] px-2 py-1.5 rounded shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 flex flex-col items-center mb-5"
                                style={{ bottom: `${heightPercent}%` }}
                            >
                                <span className="font-bold">{p.program}</span>
                                <span className="text-[9px] opacity-90">Count: {p.count}</span>
        ...
        <div className="flex h-full bg-gray-50 overflow-hidden relative">
            ...
            <div className={`${showExportPanel ? 'w-44 md:w-64 border-r' : 'w-0'} bg-white border-gray-200 flex flex-col transition-all duration-300 shrink-0 overflow-hidden`}>
                <div className="p-4 border-b border-gray-200 bg-gray-50">
                    ...
                    <div className="text-[10px] text-gray-500 font-medium mb-1">{selectedExportCols.size} / {EXPORT_COLUMNS.length}</div>
                ...
                <div className="p-4 border-t border-gray-200 bg-gray-50">
                    <div className="mb-2">
                        <input type="text" value={exportFilename} onChange={(e) => setExportFilename(e.target.value)} className="w-full text-xs border border-gray-300 rounded px-2 py-1 focus:border-blue-500 outline-none" />
                    </div>
                ...
                <div className="hidden md:flex flex-col flex-1 overflow-y-auto p-2 space-y-2 thin-scrollbar">
                    ...
                    <div className="bg-white p-2 rounded border border-gray-200 shadow-sm mt-2 flex flex-col shrink-0 relative">
                        ...
                        <div className="w-full relative h-[220px] mt-2">
                            {renderChart(sortedFaculties)}
                        </div>
                        <div className="flex justify-center flex-wrap gap-4 mt-2 border-t border-gray-100 pt-2 shrink-0">
                            {sortedFaculties.map(fac => (
                                <div key={fac} className="flex items-center space-x-1.5">
                                    <div className={`w-3 h-3 rounded-sm ${FACULTY_COLORS[fac]}`}></div>
                                    <span className="text-[10px] font-bold text-gray-600">{fac}</span>
                                </div>
                            ))}
                        </div>
                    </div>
