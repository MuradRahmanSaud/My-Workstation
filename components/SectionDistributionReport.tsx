...
interface SectionDistributionReportProps {
...
const EXPORT_COLUMNS = [
    ...
    { key: 'Employee Name', label: 'TEACHER NAME' },
...
export const SectionDistributionReport: React.FC<SectionDistributionReportProps> = ({ data, programData, lowStudentThreshold = 0, showExportPanel = false, setShowExportPanel }) => {
    const [selectedExportCols, setSelectedExportCols] = useState<Set<string>>(new Set(EXPORT_COLUMNS.map(c => c.key)));
    const [exportFilename, setExportFilename] = useState('Section_List_Report');
    ...
    const { facultyGroups, allPrograms } = useMemo(() => {
        const stats = new Map<string, { pid: string, program: string, faculty: string, totalSections: number, totalSectionsTotal: number, totalStudents: number, totalCapacity: number }>();
        data.forEach(row => {
            ...
            if (!stats.has(programName)) stats.set(programName, { pid: row.PID, program: programName, faculty: faculty, totalSections: 0, totalSectionsTotal: 0, totalStudents: 0, totalCapacity: 0 });
            const entry = stats.get(programName)!;
            entry.totalSectionsTotal += 1;
            const stud = parseInt(row.Student || '0', 10);
            if (stud >= lowStudentThreshold) {
                entry.totalSections += 1;
                entry.totalStudents += stud;
                const cap = parseInt(row.Capacity || '0', 10);
                if (!isNaN(cap)) entry.totalCapacity += cap;
            }
        });
        ...
        stats.forEach(val => {
            ...
            const item = { program: val.program, faculty: val.faculty, count: val.totalSections, totalCount: val.totalSectionsTotal, students: val.totalStudents, capacity: val.totalCapacity };
    ...
    }, [data, programMap, lowStudentThreshold]);
    ...
    const handleDownload = () => {
        ...
        try {
            const filteredData = data.filter(row => {
                const stud = parseInt(row.Student || '0', 10);
                return (isNaN(stud) ? 0 : stud) >= lowStudentThreshold;
            });
            const exportData = filteredData.map(item => {
                ...
                let percentage = 0;
                if (req > 0) percentage = (taken / req) * 100; else if (taken > 0) percentage = 100;
            ...
            (window as any).XLSX.utils.book_append_sheet(workbook, worksheet, "Section List");
    ...
    const renderFacultyCard = (fac: string, mobile: boolean = false) => {
        ...
        const totalOffered = programs.reduce((acc, p) => acc + p.count, 0);
        ...
        return (
            <div className={`bg-white rounded border shadow-sm flex flex-col overflow-hidden ${mobile ? 'w-full h-auto mb-4' : 'min-w-[300px] md:min-w-0 snap-center'}`}>
                {!mobile && (
                    <div className="py-3 md:py-1.5 px-3 md:px-2 text-center border-b font-bold text-base md:text-xs text-gray-700 bg-gray-50">{fac}</div>
                )}
                <div className={`flex justify-between px-4 md:px-2 py-2 md:py-1 ${headerColor} text-white text-sm md:text-[11px] font-bold`}>
                    <span style={{ width: '70%' }}>Program</span>
                    <span style={{ width: '30%', textAlign: 'right' }}>Section</span>
                </div>
                <div className="divide-y divide-gray-100 overflow-y-auto thin-scrollbar" style={{ height: mobile ? 'auto' : `${contentHeight}px`, maxHeight: mobile ? '400px' : 'none' }}>
                    {programs.map(p => (
                        <div key={p.program} className="flex justify-between px-4 md:px-2 py-2 md:py-0.5 text-sm md:text-[12px] hover:bg-gray-50 items-center border-b border-gray-50 last:border-0 md:border-none md:h-[27px]">
                            <span className="text-gray-700 truncate mr-2" style={{ width: '70%' }} title={p.program}>{p.program}</span>
                            <span className="font-bold text-gray-900" style={{ width: '30%', textAlign: 'right' }}>{p.count}</span>
                        </div>
                    ))}
                </div>
                <div className="bg-cyan-50 border-t border-cyan-100 px-4 md:px-2 py-2 md:py-1 flex justify-between text-sm md:text-[12px] font-bold text-gray-800">
                    <span style={{ width: '70%' }}>Total</span>
                    <span style={{ width: '30%', textAlign: 'right' }}>{totalOffered}</span>
                </div>
            </div>
        );
    };
                    ...
                    return (
                        <div key={idx} className="flex-1 flex flex-col items-center justify-end group relative h-full hover:bg-gray-50/50 rounded-lg transition-colors min-w-[20px]">
                            ...
                            <div className="absolute left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] px-2 py-1.5 rounded shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 flex flex-col items-center mb-5" style={{ bottom: `${heightPercent}%` }}>
                                ...
                                <span className="text-[9px] opacity-90">Sections: {p.count}</span>
        ...
        <div className="flex h-full bg-gray-50 overflow-hidden relative">
            ...
            <div className="flex-1 flex flex-col overflow-hidden min-w-0">
                <div className="flex justify-between items-center p-2 bg-white border-b border-gray-200 shrink-0">
                    <h3 className="text-sm md:text-lg font-bold text-gray-800 truncate mr-2">
                        Section Dist. {lowStudentThreshold > 0 && <span className="ml-1 text-[10px] font-normal text-gray-500">(&lt;{lowStudentThreshold})</span>}
                    </h3>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-4 bg-gray-50">
                        ...
                        {activeFaculty && (
                            <div className="bg-white p-2 rounded border border-gray-200 shadow-sm relative h-[300px]">
                                <h4 className="text-[10px] font-bold text-gray-500 mb-2 uppercase flex items-center"><BarChart3 className="w-3 h-3 mr-1"/> {activeFaculty} Sections</h4>
                ...
                <div className="hidden md:flex flex-col flex-1 overflow-y-auto p-2 space-y-2 thin-scrollbar">
                    <div className="flex flex-nowrap overflow-x-auto gap-3 pb-4 md:grid md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 items-start shrink-0">
