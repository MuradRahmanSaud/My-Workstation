...
const GroupHeader: React.FC<{ title: string; Icon: React.ElementType; themeClasses: { text: string; border: string; icon: string; }; onCopy: () => Promise<void>; }> = ({ title, Icon, themeClasses, onCopy }) => {
    ...
    return (
        <div className={`flex items-center justify-between border-b ${themeClasses.border} pb-1.5 mb-2`}>
            <h4 className={`text-[11px] font-bold ${themeClasses.text} uppercase tracking-wider flex items-center`}><Icon className={`w-3.5 h-3.5 mr-2 ${themeClasses.icon}`} />{title}</h4>
            <button onClick={handleClick} className={`p-1 rounded hover:bg-white/50 transition-all ${themeClasses.text} opacity-70 hover:opacity-100`}>{copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}</button>
        </div>
    );
};

const EmployeeCard: React.FC<{ title: string; employee: DiuEmployeeRow | undefined; fallbackName: string; fallbackId: string; roleLabel?: string; themeClasses?: { bg: string; border: string; icon: string; text: string; }, tableHeaderColor: string; onEdit?: () => void; }> = ({ title, employee, fallbackName, fallbackId, roleLabel, themeClasses, tableHeaderColor, onEdit }) => {
    const [copied, setCopied] = useState(false);
    ...
    // Swapped to show Academic first
    const designation = employee ? [employee['Academic Designation'], employee['Administrative Designation']].filter(Boolean).join(', ') : (employee?.['Academic Designation'] || '');
    ...
    const handleCopyDetails = async () => {
        await copyToClipboardWithTable(`${title} Details`, [{ label: 'Name', value: name }, { label: 'Designation', value: designation }, { label: 'ID', value: id }, { label: 'Mobile', value: mobile }], tableHeaderColor);
        setCopied(true); setTimeout(() => setCopied(false), 2000);
    };
    const handleCopyField = (text: string, fieldType: string) => { if (!text) return; navigator.clipboard.writeText(text).then(() => { setFieldCopied(fieldType); setTimeout(() => setFieldCopied(null), 1500); }); };
    return (
        <div className={`${theme.bg} rounded-lg border ${theme.border} shadow-inner p-3 relative overflow-hidden transition-all hover:bg-opacity-100 bg-opacity-60`}>
            <div className={`flex items-center justify-between border-b ${theme.border} border-opacity-50 pb-1.5 mb-3`}>
                <h4 className={`text-[11px] font-bold ${theme.text} uppercase tracking-wider flex items-center`}><User className={`w-3.5 h-3.5 mr-2 ${theme.icon}`} />{title}</h4>
                <div className="flex items-center space-x-1">{onEdit && <button onClick={onEdit} className={`p-1 rounded hover:bg-white/50 transition-all ${theme.text} opacity-70 hover:opacity-100`}><Pencil className="w-3.5 h-3.5" /></button>}<button onClick={handleCopyDetails} className={`p-1 rounded hover:bg-white/50 transition-all ${theme.text} opacity-70 hover:opacity-100`}>{copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}</button></div>
            </div>
            {(!name && !id && id !== 'TBA') ? <div className="py-2 px-3 bg-white/50 rounded border border-gray-200 border-dashed text-center"><p className="text-xs text-gray-400 font-medium">Not Assigned</p></div> : <div><div className="flex flex-col items-center mb-3"><div className={`w-16 h-16 rounded-full border-2 p-0.5 ${getImageBorderColor(roleLabel || '')} overflow-hidden bg-white shadow-sm mb-2`}>{imageUrl ? <img src={imageUrl} alt={name} className="w-full h-full object-cover rounded-full" referrerPolicy="no-referrer" /> : <div className="w-full h-full flex items-center justify-center bg-gray-100 text-gray-400"><ImageIcon className="w-8 h-8" /></div>}</div><h3 className="text-sm font-bold text-slate-800 leading-tight text-center mb-1">{name}</h3><p className="text-[10px] font-semibold text-slate-500 text-center leading-tight px-2">{designation}</p></div><div className={`grid grid-cols-3 gap-y-3 gap-x-1 text-center border-t ${theme.border} border-opacity-50 pt-3`}><div className="px-1"><div className={`text-[9px] font-semibold ${theme.text} opacity-70 mb-0.5 uppercase`}>Dept</div><div className="text-[10px] font-bold text-slate-700 truncate">{employee?.Department || '-'}</div></div><div className={`px-1 border-l ${theme.border} border-opacity-50`}><div className={`text-[9px] font-semibold ${theme.text} opacity-70 mb-0.5 uppercase`}>ID</div><div className="text-[10px] font-bold text-slate-700 truncate">{id || '-'}</div></div><div className={`px-1 border-l ${theme.border} border-opacity-50 cursor-pointer hover:bg-white/40 transition-colors rounded`} onClick={() => handleCopyField(mobile, 'mobile')}><div className={`text-[9px] font-semibold ${theme.text} opacity-70 mb-0.5 uppercase`}>Mobile {fieldCopied === 'mobile' && <Check className="w-2.5 h-2.5 ml-1 text-green-600" />}</div><div className="text-[10px] font-bold text-slate-700 truncate">{mobile || '-'}</div></div></div></div>}
        </div>
    );
};
...
export const SectionDetailsPanel: React.FC<SectionDetailsPanelProps> = ({ section, programData = [], employeeData = [], onClose }) => {
    ...
    const handleEditEmployee = (empData: any) => { setEmployeeEditData(empData); setIsEmployeeModalOpen(true); };
    const handleEmployeeModalSuccess = (newData: any) => { if (!newData) return; updateDiuEmployeeData(prev => prev.map(emp => normalizeId(emp['Employee ID']) === normalizeId(newData['Employee ID']) ? { ...emp, ...newData } : emp)); };

    const vacancy = (parseInt(section.Capacity || '0', 10) - parseInt(section.Student || '0', 10)) || 0;
    ...
    return (
        <div className="fixed inset-0 md:static z-[100] md:z-auto bg-black/60 md:bg-transparent backdrop-blur-sm md:backdrop-blur-none flex items-end md:items-start justify-center md:flex-col w-full md:w-[350px] lg:w-[400px] shrink-0 animate-in fade-in md:animate-none">
            <div className="w-full h-[90vh] md:h-full bg-white rounded-t-2xl md:rounded-none md:rounded-l-lg shadow-2xl flex flex-col overflow-hidden font-sans border-l border-gray-200">
                <div className="px-4 py-4 border-b border-gray-100 bg-white shrink-0 relative text-center shadow-sm">
                    <div className="absolute top-2 right-2 flex items-center space-x-1">
                        {!isEditing && (
                            <button onClick={() => setIsEditing(true)} className="p-1.5 bg-blue-50 text-blue-600 rounded-full transition-colors"><Pencil className="w-4 h-4" /></button>
                        )}
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-gray-50 thin-scrollbar">
                    ...
                    <div className={`bg-fuchsia-50/40 rounded-lg border ${isEditing ? 'border-blue-300 ring-1 ring-blue-100' : 'border-fuchsia-100'} p-3 shadow-inner`}>
                        ...
                        <div className="grid grid-cols-3 gap-1 text-center divide-x divide-fuchsia-200/30">
                            <div><div className="text-[9px] font-bold text-fuchsia-400 uppercase">Capacity</div>{isEditing ? <input value={editCapacity} onChange={e => setEditCapacity(e.target.value)} className="w-full text-xs text-center border rounded px-1 py-1 focus:border-blue-500 outline-none" /> : <div className="text-xs font-bold">{section.Capacity}</div>}</div>
                    ...
                    <EmployeeCard title="Course Teacher" employee={teacherEmployee} fallbackName={section['Employee Name'] || ''} fallbackId={section['Teacher ID']} tableHeaderColor="#b45309" themeClasses={{ bg: 'bg-amber-50/40', border: 'border-amber-100', icon: 'text-amber-500', text: 'text-amber-700' }} onEdit={teacherEmployee ? () => handleEditEmployee(teacherEmployee) : undefined} />
                    {adminEmployees.map((admin, idx) => (
                        <EmployeeCard key={idx} title="Coordination Officer" employee={admin.emp} fallbackName={admin.fallbackName} fallbackId={admin.rawId} tableHeaderColor="#0369a1" themeClasses={{ bg: 'bg-sky-50/40', border: 'border-sky-100', icon: 'text-sky-500', text: 'text-sky-700' }} onEdit={admin.emp ? () => handleEditEmployee(admin.emp) : undefined} />
                    ))}
                </div>

                {isEditing && (
                    <div className="px-5 py-4 border-t border-gray-100 bg-white flex space-x-3 shrink-0 pb-8 md:pb-4">
                        <button onClick={() => setIsEditing(false)} className="flex-1 flex items-center justify-center px-4 py-2.5 text-sm font-bold text-gray-600 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 transition-colors">
                            <Undo2 className="w-4 h-4 mr-2" /> Cancel
                        </button>
                        <button onClick={handleSave} className="flex-1 flex items-center justify-center px-4 py-2.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-md active:scale-95 transition-all">
                            <Save className="w-4 h-4 mr-2" /> Save Changes
                        </button>
                    </div>
                )}
            </div>
            <EditEntryModal isOpen={isEmployeeModalOpen} onClose={() => setIsEmployeeModalOpen(false)} mode="edit" title="Edit Employee" sheetName={SHEET_NAMES.EMPLOYEE} columns={['Employee ID', 'Employee Name', 'Academic Designation', 'Administrative Designation', 'Department', 'Group Name', 'Status', 'Mobile', 'E-mail', 'Photo', 'Facebook', 'Linkedin']} initialData={employeeEditData} keyColumn="Employee ID" spreadsheetId={REF_SHEET_ID} onSuccess={handleEmployeeModalSuccess} />
        </div>
    );
};
