...
import { ClassRoomDataRow, ProgramDataRow, CourseSectionData } from '../types';
...
interface ClassRoomDistributionReportProps {
...
const EXPORT_COLUMNS = [
    ...
    { key: 'Slot Duration', label: 'SLOT DURATION' },
    { key: 'Slot Per Room', label: 'SLOT PER ROOM' },
    { key: 'Shared Program', label: 'SHARED PROGRAM' },
];
...
export const ClassRoomDistributionReport: React.FC<ClassRoomDistributionReportProps> = ({ 
    data, 
    programData = [], 
    sectionData = [],
    semesterFilter,
    ...
    const [exportFilename, setExportFilename] = useState('Class_Room_Report');
    ...
    const { facultyGroups, sortedFaculties, allPrograms } = useMemo(() => {
        const groups: Record<string, { program: string, pid: string, count: number }[]> = {};
        const progCounts = new Map<string, { faculty: string, program: string, pid: string, count: number }>();
        const allProgramsList: { program: string, faculty: string, count: number }[] = [];

        data.forEach(row => {
            const pid = String(row.PID || '').trim();
            const normalizedPid = pid.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
            let faculty = 'Other', programName = pid || 'Common/Shared';
            if (pid) { const info = programMap.get(normalizedPid); if (info) { faculty = info.faculty; programName = info.shortName; } }
            const key = `${faculty}||${programName}||${pid}`;
            if (!progCounts.has(key)) progCounts.set(key, { faculty, program: programName, pid: pid, count: 0 });
            progCounts.get(key)!.count++;
        });

        progCounts.forEach(({ faculty, program, pid, count }) => {
            if (!groups[faculty]) groups[faculty] = [];
            groups[faculty].push({ program, pid, count });
            allProgramsList.push({ program, faculty, count });
        });
        Object.keys(groups).forEach(f => groups[f].sort((a, b) => a.program.localeCompare(b.program)));
        const sorted = ['FBE', 'FE', 'FHLS', 'FHSS', 'FSIT'].filter(f => groups[f]);
        const others = Object.keys(groups).filter(f => !['FBE', 'FE', 'FHLS', 'FHSS', 'FSIT'].includes(f)).sort();
        return { facultyGroups: groups, sortedFaculties: [...sorted, ...others], allPrograms: allProgramsList };
    }, [data, programMap]);

    const modalData = useMemo(() => {
        if (!selectedProgram) return { rooms: [], sections: [] };
        const normalize = (id: string) => String(id || '').replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
        const targetPidRaw = String(selectedProgram.id);
        const targetPidNorm = normalize(targetPidRaw);
        const targetPidInt = parseInt(targetPidRaw, 10);
        const isTargetNumeric = !isNaN(targetPidInt);

        const match = (pid: string) => {
            if (!pid) return false;
            const pidStr = String(pid);
            if (pidStr === targetPidRaw) return true;
            const pNorm = normalize(pidStr);
            if (pNorm === targetPidNorm) return true;
            if (isTargetNumeric) {
                const pidClean = pidStr.replace(/^0+/, ''), targetClean = targetPidRaw.replace(/^0+/, '');
                if (pidClean === targetClean) return true;
                const pInt = parseInt(pidStr, 10);
                if (!isNaN(pInt) && pInt === targetPidInt) return true;
            }
            return false;
        };
        const rooms = data.filter(r => match(r.PID));
        const safeSectionData = Array.isArray(sectionData) ? sectionData : [];
        const sections = safeSectionData.filter(s => match(s.PID));
        return { rooms, sections };
    }, [selectedProgram, data, sectionData]);
    ...
    const maxProgramsCount = useMemo(() => {
        ...
        return Math.max(...sortedFaculties.map(f => facultyGroups[f]?.length || 0));
    ...
    // Added handleSelectAll and handleDeselectAll functions to fix undefined errors
    const handleSelectAll = () => setSelectedExportCols(new Set(EXPORT_COLUMNS.map(c => c.key)));
    const handleDeselectAll = () => setSelectedExportCols(new Set());
    ...
    const handleDownload = () => {
        ...
        try {
                ...
                EXPORT_COLUMNS.forEach(col => {
                    if (selectedExportCols.has(col.key)) {
                        if (col.key === 'Program') {
                            const pid = item.PID;
                            const normalize = (id: string) => String(id || '').replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
                            const info = programMap.get(normalize(pid));
                            row[col.label] = info ? `${pid} ${info.shortName}` : pid;
                        } else {
                            // @ts-ignore
            ...
            (window as any).XLSX.utils.book_append_sheet(workbook, worksheet, "Class Rooms");
    ...
    const renderFacultyCard = (fac: string, mobile: boolean = false) => {
        ...
        return (
            <div className={`bg-white rounded border shadow-sm flex flex-col overflow-hidden ${mobile ? 'w-full h-auto mb-4' : 'min-w-[250px] md:min-w-0 snap-center'}`}>
                ...
                <div className="divide-y divide-gray-100 overflow-y-auto thin-scrollbar" style={{ height: mobile ? 'auto' : `${contentHeight}px`, maxHeight: mobile ? '400px' : 'none' }}>
                    {programs.map(p => (
                        <div 
                            key={p.program} 
                            onClick={() => setSelectedProgram({ id: p.pid, name: p.program })}
                            className="flex justify-between px-3 md:px-2 py-1.5 md:py-0.5 text-sm md:text-[12px] hover:bg-blue-50 cursor-pointer items-center border-b border-gray-50 last:border-0 md:border-none md:h-[27px] transition-colors"
    ...
    const renderChart = (facultiesToShow: string[]) => {
        ...
        return (
            <div className="absolute inset-0 flex items-end justify-between space-x-1 px-2 pb-2 pt-12 overflow-x-auto thin-scrollbar">
                {chartData.map((p, idx) => {
                    const heightPercent = Math.max((p.count / maxCount) * 65, 2); 
                    ...
                    return (
                        <div key={idx} className="flex-1 flex flex-col items-center justify-end group relative h-full hover:bg-gray-50/50 rounded-lg transition-colors min-w-[20px]">
                            <span className="text-[9px] font-bold text-gray-600 mb-0.5">{p.count}</span>
                            <div className="absolute left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] px-2 py-1.5 rounded shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 flex flex-col items-center mb-7" style={{ bottom: `${heightPercent}%` }}>
                                <span className="font-bold">{p.program}</span>
                                <span className="text-[9px] opacity-90">Rooms: {p.count}</span>
    ...
    return (
        <div className="flex h-full bg-gray-50 overflow-hidden relative">
            ...
            <div className={`${showExportPanel ? 'w-44 md:w-64 border-r' : 'w-0'} bg-white border-gray-200 flex flex-col transition-all duration-300 shrink-0 overflow-hidden`}>
                <div className="p-4 border-b border-gray-200 bg-gray-50">
                    ...
                    <div className="flex space-x-2 mt-2">
                         <button onClick={handleSelectAll} className="flex-1 py-1 text-[10px] bg-white border border-gray-300 rounded hover:bg-gray-50 text-gray-600">All</button>
                         <button onClick={handleDeselectAll} className="flex-1 py-1 text-[10px] bg-white border border-gray-300 rounded hover:bg-gray-50 text-gray-600">None</button>
                    </div>
                </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden min-w-0">
                <div className="flex justify-between items-center mb-1 p-2 bg-white border-b border-gray-200 shrink-0">
                    <h3 className="text-sm md:text-lg font-bold text-gray-800 truncate mr-2">Class Room Dist.</h3>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-4 bg-gray-50">
                        ...
                        {activeFaculty && (
                            <div className="bg-white p-2 rounded border border-gray-200 shadow-sm relative h-[300px]">
                                <h4 className="text-[10px] font-bold text-gray-500 mb-2 uppercase flex items-center"><BarChart3 className="w-3 h-3 mr-1"/> {activeFaculty} Rooms</h4>
            ...
            {selectedProgram && <ProgramAnalysisModal isOpen={!!selectedProgram} onClose={() => setSelectedProgram(null)} programName={selectedProgram.name} programId={selectedProgram.id} classroomData={modalData.rooms} sectionData={modalData.sections} lowStudentThreshold={7} selectedSemester={semesterFilter} />}
            {showExportPanel && <div className="fixed inset-0 bg-black/20 z-20 md:hidden backdrop-blur-sm" onClick={() => setShowExportPanel && setShowExportPanel(false)}></div>}
        </div>
    );
};
